...
- **modelPolicy.allow 写入安全**：Core 是唯一 writer；自动同步路径仅在 policy 已存在且非空时同步 exact entries，绝不创建、清空、隐式展开、删除或改写用户 wildcard。单模型/Provider disable、rename、batch cleanup 若无法不改 wildcard 地表达，必须 fail closed，`force` 也不可绕过。**显式用户指令的规则编辑是独立入口**（`addModelPolicyRule` / `removeModelPolicyWildcard`，见 2026-09-13 policy editing spec）：仅限 restricted 模式（绝不隐式创建 policy），删除 wildcard 受防清空与 primary/fallback 覆盖保护，fail closed。per-agent `agents.entries.*.modelPolicy.allow` 不在范围内。`config-status` 对 legacy `agents.defaults.models` metadata ref 未被非空 policy 覆盖，必须产生 `health:model-policy-not-covered:modelPolicy.allow`（source=`health`、severity=`warning`）全局 issue，并在 detail/action 中说明 metadata 不参与 restricted selection 及修复方向。
