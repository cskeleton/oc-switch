...
- 每 Provider 本地目录硬上限 50；存量超过 50 仍可用，但禁止再添加，直到删到少于 50
...
- 不把「目录超过 50」做成 `config-status` 强制 issue（存量只拦新增）
...
- 每 Provider 50 条上限（`MAX_PROVIDER_MODELS`）不变，且建议查询不影响该上限。
...
| 硬上限 | 每 Provider 已添加条数上限 | 常量 `MAX_PROVIDER_MODELS = 50` |
...
常量：`MAX_PROVIDER_MODELS = 50`，含义为单个 Provider 的 `models.providers[id].models.length`。

**必须在 core 增加模型目录条数的路径上统一校验**（推荐抽 `assertProviderModelCapacity(provider, addingCount)`，在 mutate 前调用）。至少覆盖：

| 写入入口 | 现状参考 | 要求 |
|----------|----------|------|
| `batch-add` / CLI `provider sync --add` | 本规格新增 | `current + newlyAdded ≤ 50` |
| `addProviderModel` / `POST /api/models` / CLI `model add` | `model-operations.ts` | 单次 +1 时若已达 50 则拒绝 |
| `addCustomProvider` / custom 创建 | `provider-operations.ts` | 初始 `models.length` 不得超过 50 |
| preset / import 等会写入或扩张 `provider.models` 的路径 | 现有 import/preset 流程 | 扩张后总数不得超过 50；失败整单拒绝 |

规则细节：

- **存量已超过 50**：不自动删；允许删除、启停、编辑已有条目；**任何净增加条数的操作拒绝**，错误信息提示先删除或「只保留已启用」
- **替换/编辑**（改 id、改字段但不增加条数）：不受「再 +N」限制；若编辑把 id 改成新 id 且旧 id 移除，条数不变则允许
- **前端/CLI 预检**可有，但**不能替代** core 校验
- 不把「目录超过 50」做成 `config-status` 强制 issue（与 §3 一致）
...
- 校验：`currentCount + newlyAddedCount ≤ 50`（及 §5.1）；失败整单拒绝，不部分写入
...
| `oc-switch provider sync <id> --add id1,id2` | 等价 batch-add（仅 id，无 name 则默认命名）；id 为 raw model id，可含 `/`；尊重上限 50 |
...
- 启停：避免整页暴力刷新；优先本地 state 更新或仅重拉该 Provider 模型（已添加规模按上限 50 设计）

### 9.4 存量超过 50

- 不自动清理、不做健康检查强制项
- 发现/添加在已达 20 时禁止新增
- 用户用批量删除或「只保留已启用」清到不超过 50
...
- [ ] 每 Provider 目录总数不可超过 50；`batch-add`、单条 `model add`、`add-custom`、import 等写入入口均不可绕过；存量超过 50 可删不可增
