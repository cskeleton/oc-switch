 ...
 * （`context.pluginCatalog()`），未知 id → 清晰报错 + 非零退出、不写任何文件——镜像
 ...
 * `writeOpenClawTransaction`（自动备份 + diff guard），只改
 * `plugins.entries.<id>.enabled` 一个键。与 `sync push --enable-plugins` 的
...
async function changePluginState(
  ...
  try {
    ...
    const currentDescriptor = (): ModelPluginDescriptor => {
      context.invalidateCatalogCaches();
      const catalog = context.pluginCatalog(paths);
    ...
    try {
      const inventory = context.buildInventory({ refresh: true, paths });
      diagnostics = inventory.diagnostics;
      runtimeConfirmed = diagnostics.length === 0 && Object.values(context.runtimeModelSnapshot(paths).completeness).every(Boolean) &&
...
export function registerPluginCommands(program: Command, context: CommandContext): void {
  const plugin = program
    .command("plugin")
    .description("模型插件启停（只改 plugins.entries.<id>.enabled 一个键，policy 原样保留）");
  ...
  plugin.command("enable")
    ...
    .action(async (pluginId: string, options: { yes?: boolean; json?: boolean }) => {
